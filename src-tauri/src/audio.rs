...
pub fn write_audio(tts: &TtsResult, out_dir: &Path, file_stem: &str, format: AudioFormat) -> Result<WrittenAudio> {
    ...
    match format {
        AudioFormat::Wav => Ok(WrittenAudio { path: wav_path, duration_ms }),
        AudioFormat::Mp3 | AudioFormat::Ogg => {
            ...
            Ok(WrittenAudio { path: final_path, duration_ms })
        }
    }
}
...
fn pcm_duration_ms(tts: &TtsResult) -> u64 {
    ...
    let total_samples = if bytes_per_sample == 0 { 0 } else {
        tts.pcm_bytes.len() as u64 / bytes_per_sample / tts.channels as u64
    };
    if tts.sample_rate == 0 { 0 } else { total_samples * 1000 / tts.sample_rate as u64 }
}

pub fn ensure_ffmpeg() -> Result<()> {
    let probe = Command::new("ffmpeg").arg("-version").stdout(Stdio::null()).stderr(Stdio::null()).status();
    match probe {
...
pub fn convert_audio_file(src: &Path, dst: &Path, format: AudioFormat) -> Result<()> {
    if format == AudioFormat::Wav && src.extension().and_then(|e| e.to_str()) == Some("wav") {
        std::fs::copy(src, dst).with_context(|| format!("cannot copy {} to {}", src.display(), dst.display()))?;
        return Ok(());
    }
    ensure_ffmpeg()?;
...
fn convert_with_ffmpeg(src: &Path, dst: &Path, format: AudioFormat) -> Result<()> {
    ...
    cmd.arg("-y").arg("-loglevel").arg("error").arg("-i").arg(src);
    match format {
        AudioFormat::Mp3 => { cmd.arg("-codec:a").arg("libmp3lame").arg("-q:a").arg("2"); }
        AudioFormat::Ogg => { cmd.arg("-codec:a").arg("libvorbis").arg("-q:a").arg("5"); }
        AudioFormat::Wav => {}
    }
    cmd.arg(dst);
