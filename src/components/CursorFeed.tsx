...
export default function CursorFeed({ session, archive, currentId, onPlay }: Props) {
  const items = useMemo(() => {
    const merged = [...session, ...archive].filter((g) => g.source === "cursor");
  ...
  const stats = useMemo(() => {
    ...
    const recent = items.filter((g) => g.created_at >= dayAgo);
    ...
    <li>
      ...
      >
        ...
        <div className="text-[10px] text-muted">
          {gen.voice} · {gen.format.toUpperCase()}
